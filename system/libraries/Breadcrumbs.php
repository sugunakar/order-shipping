...
class CI_Breadcrumbs {
	 ...
	 * @return	void
	 */		
	function push($page, $href, $symbol)
	{
		// no page or href provided
		if (!$page OR !$href) return;
		
		// push breadcrumb
		$this->breadcrumbs[] = array('page' => $page, 'href' => $href, 'symbol' => $symbol);
	}
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				if (@end(array_keys($this->breadcrumbs)) == $key) {
					$output .= $this->crumb_open.$crumb['symbol'].' '.$crumb['page'] .$this->crumb_close;
				} else {
					$output .= $this->crumb_open.$crumb['symbol'].' <a href="' . $crumb['href'] . '">' . $crumb['page'] . '</a> '.$this->crumb_divider.$this->crumb_close;
				}
			}
